...
of care to those in need.
This map is a basic exploration into the number of children's hospitals per state.
It shows the number per state, with Texas having 16 hospitals and California having 11, while many states have no hospital.
Some states have many children's hospitals, others have none, creating disadvantaged populations in those states in particular.

![](http://drdavis.space/teaching/ds/sdd/media/2019/12/childrens_hospitals_heatmap.png)

In states where there are children's hospitals, they are generally centralized darkest green areas in the map above show where hospitals are centralized in
each state, while the lighter areas are where there are less hospitals.
White areas indicate the absence of children's hospitals in general.
...
Three D rendering was used to show the number of hospitals per state and a video was taken of the rendering.
