...
Each HDF4 file contained multiple subdatasets, data from "Majority_Land_Cover_Type_1" variable were extracted from each file and put into yearly raster files (.txt) in ASCII format with a computer program written in Python[^python] with the assistance of gdal[^gdal]. These arrays of data were also put into a new HDF5 file that can be easily viewed, each year a different dataset, as the original files were large and contained excess information not relevant to this project. To better show urbanization and development of land, all undeveloped land cover types were reclassified as value 1, cropland as value 2, and urban land cover as 3. Water remained the same as 0.
